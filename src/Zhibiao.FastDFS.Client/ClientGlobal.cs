...
namespace Zhibiao.FastDFS.Client
{
    ...
    public class ClientGlobal
    {
        ...
        public static void init(Action<FastDFSOptions> config)
        {
            lock (lockObj)
            {
                if (isInit) return;
                ...
                if (config == null)
                {
                    throw new ArgumentNullException("config", "Tracker servers must be configured");
                }

                var options = new FastDFSOptions();
                config.Invoke(options);

                if (options.Trackers == null || options.Trackers.Count() == 0)
                {
                    throw new Exception("tracker server not found");
                }

                var connectTimeout = options.ConnectTimeout > 0 ? options.ConnectTimeout : DEFAULT_CONNECT_TIMEOUT;
                g_connect_timeout = connectTimeout * 1000;

                var networkTimeout = options.NetworkTimeout > 0 ? options.NetworkTimeout : DEFAULT_NETWORK_TIMEOUT;
                g_network_timeout = networkTimeout * 1000;

                g_charset = string.IsNullOrWhiteSpace(options.Charset) ? "ISO8859-1" : options.Charset.Trim();
                g_tracker_group = new TrackerGroup(options.Trackers
                    .Select(x => new IPEndPoint(IPAddress.Parse(x.IP.Trim()), x.Port))
                    .ToArray());
                g_tracker_http_port = options.TrackerHttpPort > 0 ? options.TrackerHttpPort : 80;
                g_anti_steal_token = options.AntiStealToken;

                if (g_anti_steal_token)
                {
                    g_secret_key = options.SecretKey;
                }
